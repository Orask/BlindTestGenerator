...
| Thème "Génériques dessins animés/films" n'a que 12 des 18 artistes curatés qui remontent un résultat exploitable sur Spotify (~24 morceaux max avec le plafond de 2/artiste, mesuré le 2026-08-08 après le correctif de matching exact — section 3septies), sous les 60 requis | `InsufficientTracksError` au premier dimanche si non corrigé                                                     | Enrichir `seedArtists` de ce thème avant dimanche (viser ~30 artistes qui remontent réellement des résultats), ou réduire le nombre de morceaux spécifiquement pour ce thème en attendant |
